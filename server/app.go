...
func (server *Server) Init(user string, password string, dbname string, dbhost string) {

	connectionValues := fmt.Sprintf("user=%s password=%s dbname=%s host=%s sslmode=disable", user, password, dbname, dbhost)
